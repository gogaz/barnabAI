...
class CodeownersMatcher
  ...
  def determine_impacted_teams(files)
    return [] unless files&.any?

    # Only check CODEOWNERS for matera-tech/api-core repository
    return [] unless @repository.full_name == "matera-tech/api-core"

    # Fetch CODEOWNERS file
    codeowners_content = fetch_codeowners
    ...
    files.each do |file|
      ...
      # Normalize filename: CODEOWNERS patterns use absolute paths (starting with /)
      # but GitHub PR file paths are relative (no leading /)
      normalized_filename = filename.start_with?("/") ? filename : "/#{filename}"

      # Match file against CODEOWNERS rules
